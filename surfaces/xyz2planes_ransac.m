...
function [planes, plane_idx] = xyz2planes_ransac(X, Y, Z, normals, isvalid)
  ...
  if DEBUG_ 
    for t = 1:size(planes, 1)
        ...
        tmpim = zeros(H, W);
        tmpim(in) = 1;        
        h = figure('Visible','off'); hold off, imshow(tmpim);  title('thresholded planes');
        if params.degub_fig
          saveas(h, sprintf('%s%06d_a_ransac_%03d.fig',consts.planeDataDir,ii_,t), 'fig');
        end
        print(h, '-dpng', sprintf('%s%06d_a_ransac_%03d.png',consts.planeDataDir,ii_,t));
        
        %h2_ = figure(5); imshow(repmat(tmpim, [1 1 3]).*in); title('remapped onto 3d');
        %saveas(h2_, sprintf('%s%06d_ransac_repmat_%03d.png',consts.planeDataDir,ii_,t), 'png')
        %pause(0.5);   
        close(h);
    end
  end
