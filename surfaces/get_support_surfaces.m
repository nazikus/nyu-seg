...
function supportSurfaceMap = get_support_surfaces(points3d, normals, ...
  ...
  %MARKER flip_normals_towards_interior
  ...
  while nnz(sampleGrid) > 0
    if DEBUG_
      ...
      title('supportSurfaceMap');
      if params.degub_fig
        saveas(h_, sprintf('%s%06d_e_support_surface_map_%02d.fig',consts.planeDataDir,ii_,iii_), 'fig');
      end
      print(h_, '-dpng', sprintf('%s%06d_e_support_surface_map_%02d.png',consts.planeDataDir,ii_,iii_));
      close(h_);
    end
