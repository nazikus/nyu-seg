function classifier = train_boosted_dt_2c(features, cat_features, ...
    labels, num_iterations, nodespertree, stopval, w, stage)
...
for t = 1:num_iterations
    ...
    msg = ['c: ' num2str(mean(1 ./ (1+exp(-y.*data_confidences)))) '  e: ' ...
    ...
    if t>10 && (aveconf(t)-aveconf(t-10) < stopval)
        ...
        fprintf(fdebug, '\n%s\n%s\n\n', num2str(aveconf), ['Stopping after ' num2str(t) ' trees']);
        break;
    end    
end
...
fprintf(fdebug, ['mean conf = ' num2str(mean(1 ./ (1+exp(-y.*data_confidences))))]);
fprintf(fdebug, ['training error: ' num2str(mean(y.*data_confidences < 0))]);
