function [fragments, junctions, neighbor_data, seg, avg_seg_colors, ...
    polyfragments, poly_params] = seg2fragments(seg, img, min_area, order, ii, stage)
%
...
%MARKER Assuming any segments were removed, we'll need to update the crack info
cracks = seg2cracks(seg);
...
if(DEBUG_)
    ...
    if(exist('drawedges', 'file'))
        %subplot 121, 
        drawedges(fragments, 'rand');
        %subplot 122, drawedges(polyfragments, 'rand');
    else
        %subplot 121
        for(i=1:length(fragments))
            plot(fragments{i}(:,1), fragments{i}(:,2), 'r', 'LineWidth', 0.5);
        end
%         subplot 122
...
%         end
    ...
    stage_msg = '';
    if exist('stage', 'var')
        stage_msg = sprintf('_stage%d',stage);
    end
    
    filename_ = sprintf('%s%06d_d_fragments%s',consts.watershedDir,ii,stage_msg);
    global segtestDir;
    if ~isempty(segtestDir)
