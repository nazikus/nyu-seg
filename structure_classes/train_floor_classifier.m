...
for ii = 1 : numel(lambdas)
  ...
  % outFilename = sprintf('%s\b\b\b\b_size%d.mat',sprintf(consts.floorClassifier, params.regionSrc, params.stage), length(consts.useNdx));
  global debug_stream;
  debug_stream = fopen([outFilename '.txt'], 'w'); % 1 == stdout
  %=====
