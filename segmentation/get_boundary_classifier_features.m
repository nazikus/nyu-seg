...
function features = get_features_2d(boundaryInfo, X, imgRgb, regionInteriors, regionBorders)
  ...
  features(:,2) = X.edge.length(ind) ./ minperim; % edge length / perim of smaller region (B2)
  features(:,3) = min(X.edge.smoothness(ind),1); % measure of smoothess (B3)

  % Relative angle (continuity) (B5)
...
function features = get_features_3d(boundaryInfo, X, points3d, normals, ...
  ...
  %% Geometry features
  features(:, 4) = sqrt(dX+dY+dZ); % 3D distance between bounding boxes (R4)
