...
function classifier = train_boundary_classifier_dt(stage, ...
  ...
  %MARK Train boosted decisiont tree
  classifier = train_boosted_dt_2c(trainDataSample, [], trainLabelsSample, numIterations, numNodes, stopval, w, stage); %stage passed for debugging only
  
  trainScores = test_boosted_dt_mc(classifier, trainData);
  testScores = test_boosted_dt_mc(classifier, testData);
    
  %DEBUG Plot training curve
  ...
  title(sprintf('Train AUC: %f, Test AUC: %f (Stage %d)', trainAuc, testAuc, stage));
