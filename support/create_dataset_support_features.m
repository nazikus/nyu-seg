% Creates a training and testing set using the support features and their
% labels and saves it to disk.
%
% Args:
%   params - the parameters struct (see Params.m)
%   stage - the current stage of segmentation.
function create_dataset_support_features(params, stage)
  Consts;
  ...
  for ii = 1 : consts.numImages
    ...
    [features, supporteeIds, supporterIds] = ...
        load_support_features(ii, params);
  ...
  %% Save to disk.
  outFilename = sprintf(consts.supportDataset, params.regionSrc, stage);
