...
function nn = nn_train_sgd(nn, data, labels, sampleWeights)
  ...
  global debug_stream;
%  assert(~isempty(debug_stream), 'Assert %DEGUG error, check global var');
  if isempty(debug_stream)
      debug_stream = 1;
  end
  ...
  while updatesToMake > 0
    ...
    for bb = 1 : numBatches
      ...
      fprintf(debug_stream, '(%s) Batch %d/%d, avgLoss=%f\r', nn.tag, bb, numBatches, avgLoss); 
    ...
    fprintf(debug_stream, 'Time elapsed: %3.0f hours, %3.0f minutes, %3.0f seconds.\n', elapsedTimeRun(1), elapsedTimeRun(2), elapsedTimeRun(3));
    ...
    fprintf(debug_stream, 'Time remaining: %3.0f hours, %3.0f minutes, %3.0f seconds.\n', secondsParts(1), secondsParts(2), secondsParts(3));
  end
end
